...
int main (void)
{
    ...
    pthread_create(&quitThread, NULL, quitfun, (void *)structHandle);

    structHandle -> client_data[clientsId - 1].human = 0;

    //Main game loop
    while (1)
    {
        //For now the bot is going to be very naive.
...
void * quitfun(void *arg)
{
    while (1)
    {
        ...
        if (ch == 'q' || ch == 'Q')
        {
            ((struct global_data_t *)arg) -> current_connections--;
            ((struct global_data_t *)arg) -> connected_clients[clientsId - 1] = 0;
            ((struct global_data_t *)arg) -> client_data[clientsId - 1].x = -1;
            ((struct global_data_t *)arg) -> client_data[clientsId - 1].y = -1;
